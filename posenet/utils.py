...
def get_pos_from_img(
    ...
    for ii, score in enumerate(instance_scores):
        ...
        for ks, kc in zip(keypoint_scores[ii, :], keypoint_coords[ii, :, :]):
            if ks < min_part_score:
                continue
            temp.append((int(kc[1]), int(kc[0])))
        cv_keypoints = cv_keypoints + temp[:3]

    try:
        center, l_eye, r_eye = cv_keypoints
    except ValueError:
        return (0, 0, 0, 0)

    radius = int(2 * min(dist(center, l_eye), dist(center, r_eye)))
    center = (center[0] + (l_eye[0] - center[0]) + (r_eye[0] - center[0]), center[1])
